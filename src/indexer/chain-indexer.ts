...
export class ChainIndexer {
    ...
    private async fetchBlockTimestamps(events: BlockEvent[]): Promise<Map<number, number>> {
    ...
    private async processBatch(startBlock: number, endBlock: number, retryCount: number = 0): Promise<void> {
        try {
            ...
            const [xenEvents, burnEvents, nftEvents, swapEvents, liquidityEvents] = await Promise.all([
                this.contracts?.xen.queryFilter('Transfer', startBlock, endBlock) || [],
                this.contracts?.burn.queryFilter('XENBurned', startBlock, endBlock) || [],
                this.contracts?.nft.queryFilter('BurnLockCreated', startBlock, endBlock) || [],
                this.contracts?.burn.queryFilter('SwapAndBurn', startBlock, endBlock) || [],
                this.contracts?.burn.queryFilter('LiquidityAdded', startBlock, endBlock) || []
            ]);
